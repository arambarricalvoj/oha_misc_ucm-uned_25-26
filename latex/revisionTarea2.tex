...
sobre la propuesta inicial de búsqueda local, se resuelve el problema y se exponen los resultados obtenidos.\\

Tras incorporar dichas modificaciones se observó que
...
cinemáticamente válidos, reduciendo la dimensionalidad
efectiva del problema y eliminando gran parte del ruido en la evaluación. En el
nivel 2 se emplea un Temple Simulado con
...
se han fijado las orientaciones acorde a la \autoref{fig:conf} y se obtiene el rango de posiciones del raíl
para que el robot B pueda alcanzar la posición generada. \\
...
Tras resolver exitosamente el problema con este enfoque, se ha descubierto la posibilidad de aplicar otra 
metodología:
establecer un mecanismo de evaluación que determine cómo de lejos se encuentra un punto del 
...
del robot B se encuentra desplaza ($0.75 + \textit{posición del raíl}$) metros en el eje $X$, donde el raíl puede tomar cualquier valor entre $0.0$ y $0.70$, y las orientaciones
son fijas en $\mathbf{q}_A = (0.70710678, 0.70710678, 0.0, 0.0)$ y 
$\mathbf{q}_B = (0.0, -0.38268343, 0.0, 0.92387953)$ (cumpliendo con 
la \autoref{fig:conf}). \\
...
\begin{enumerate}
    ...
    final en el mundo y la posición de enfrentamiento. Se ha establecido el \textit{offset} a 
    $0.2$ metros en el eje $x$. \\
    ...
    \item \textbf{Filtrado cinemático y manipulabilidad.}
...
el algoritmo del Temple Simulado será la sincronización temporal:
...
Por tanto, el valor \(\alpha = 0.98\) no es arbitrario, sino que garantiza un
...
\begin{figure}[H]
  \centering
  \includegraphics[scale=0.2]{Images/tema2/boxplot_Tmax.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[scale=0.18]{Images/tema2/boxplot_pA_x_pB_x.png}
...
nula. Es importante justificar el uso de $J$ para graficar esta curva, ya que 
aunque la función de mérito utilizada en el nivel 2 del Temple Simulado se reduce a la
...
\begin{figure}[H]
  \centering
  \includegraphics[scale=0.17]{Images/tema2/scatter_plot_propuesto.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[scale=0.17]{Images/tema2/convergencia_media.png}
  \caption{Curva media de convergecia del método desarrollado, donde se aprecia la disminución
...
algoritmo para explorar únicamente perfiles elevados de velocidad, reduciendo el espacio de búsqueda, provocando una convergencia prematura y priorizando la minimización del tiempo a costa de ignorar
...
Por último, otro trabajo futuro que se plantea es la resolución del problema con la metodología
